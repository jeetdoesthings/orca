...
/**
 * FrontierLabels — floating visual labels for unexplored artist nodes on the globe.
 * Displays the artist name alongside a delicate, dynamically genre-colored "unexplored" tag capsule.
...
export function FrontierLabels() {
  ...
  useFrame(() => {
    ...
    for (const [id, el] of labelsRef.current) {
      ...
      if (displaced && el.parentElement) {
        ...
        // Custom updates to R3F Drei HTML wrapper coordinate position is handled via parent transform
        // But adjusting actual DOM elements style transforms or margins for slight micro-interactions is possible.
      }
      ...
      // Zoom visibility gating: only show unexplored labels when zoomed in (camera distance < 5.2)
      ...
      {labelsData.map(label => (
        ...
        >
          ...
          >
            ...
            {/* Premium genre-colored capsule "unexplored" badge */}
            <span
              style={{
                fontSize: '8px',
                fontWeight: 700,
                ...
                color: label.genreColor,
                background: `${label.genreColor}12`, // 11% opacity background
                border: `1px solid ${label.genreColor}33`, // 20% opacity border
                borderRadius: '100px',
                padding: '1.5px 5px',
                display: 'inline-flex',
                alignItems: 'center',
